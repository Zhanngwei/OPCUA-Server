...
public class CompleteCertificateAuthTest {
    ...
    public static void testCompleteAuthentication() {
        ...
        try {
            System.out.println("=== Complete OPC UA Certificate Authentication Test ===");
            ...
            for (int i = 0; i < endpointsList.size(); i++) {
                ...
                for (UserTokenPolicy policy : policies) {
                    ...
                    // 检查是否支持证书认证
                    if (policy.getTokenType() == UserTokenType.Certificate) {
                        System.out.println("      ✅ Certificate authentication supported!");
                    }
                }
                System.out.println();
            }
            ...
            // 检查端点是否支持证书认证
            boolean supportsCertificateAuth = false;
            UserTokenPolicy[] policies = selectedEndpoint.getUserIdentityTokens();
            for (UserTokenPolicy policy : policies) {
                if (policy.getTokenType() == UserTokenType.Certificate) {
                    supportsCertificateAuth = true;
                    System.out.println("✅ Selected endpoint supports certificate authentication");
                    break;
                }
            }
            
            if (!supportsCertificateAuth) {
                System.out.println("⚠️ Selected endpoint does not support certificate authentication");
            }
            ...
            // 7. 证书认证测试（如果支持）
            if (supportsCertificateAuth) {
                System.out.println("7. Certificate authentication is supported but requires client certificates");
                System.out.println("   To test certificate authentication, you would need to:");
                System.out.println("   - Generate a client certificate");
                System.out.println("   - Add it to the server's trusted certificates");
                System.out.println("   - Configure the client to use the certificate");
            } else {
                System.out.println("7. Certificate authentication is not enabled on this endpoint");
            }
            
            System.out.println();
            System.out.println("=== Authentication Test Completed ===");
            System.out.println("✅ Server supports multiple authentication methods");
            System.out.println("✅ Certificate management system is ready for use");
